...
class TripPrediction2Hour:
    def __init__(self, seq_length=12, ensemble=False, lr=0.0005):
        ...
        self.prediction_horizon = 2  # Changed from 3 to 2 hours ahead
        self.model = None
    ...
    def plot_training_history(self, history):
        ...
        plt.tight_layout()
        plt.savefig('./training_history_2h.png')
        plt.show()
    ...
    def plot_predictions(self, y_true, y_pred, n_samples=200):
        ...
        plt.legend()
        plt.savefig('./prediction_results_2h.png')
        plt.show()
        ...
        plt.savefig('./residual_plot_2h.png')
        plt.show()
    ...
    def analyze_feature_importance(self, X_train, feature_names, n_top=15):
        ...
        plt.tight_layout()
        plt.savefig('./feature_importance_2h.png')
        plt.show()
...
def analyze_features_for_2h_prediction(model, df):
    ...
    plt.title('Feature Importance by Category (2-hour Prediction)')
    ...
    plt.tight_layout()
    plt.savefig('./feature_importance_analysis_2h.png')
    plt.show()
    ...
    plt.title('Correlation Between Top Features and Trip Count (2-hour Prediction)')
    plt.tight_layout()
    plt.savefig('./feature_correlation_heatmap_2h.png')
    plt.show()
...
# Example usage:
def run_2hour_prediction_pipeline():
    # Load data
    df = pd.read_csv('hourly_trips_with_weather.csv')
    df['datetime'] = pd.to_datetime(df['datetime'], errors='coerce')
    
    print("Training 2-Hour Prediction Model")
    model_2h = TripPrediction2Hour(seq_length=12, ensemble=True)  # Reduced sequence length
    history = model_2h.fit(df, epochs=20, batch_size=128)
    model_2h.save('./trip_prediction_2hour_model')
    
    print("\nPerforming Feature Importance Analysis")
    analyze_features_for_2h_prediction(model_2h, df)
    
    # Test prediction
    # Let's use the last 24 hours of data to predict 2 hours ahead
    ...
    print("\nComplete! 2-hour prediction model has been trained and saved.")


# Run this to train the model
if __name__ == "__main__":
    run_2hour_prediction_pipeline()