...
class TripPrediction24Hour:
    def __init__(self, seq_length=24, ensemble=False, lr=0.0005):
        self.seq_length = seq_length
        self.prediction_horizon = 24  # Fixed to predict 24 hours ahead
        self.model = None
    ...
    def preprocess_data(self, df, add_lags=True):
        ...
        if add_lags and 'trip_count' in data.columns:
            ...
            for lag in [1, 2, 3, 6, 12, 24, 48]:  # Added 48-hour lag
            ...
            data['trip_count_ma_48h'] = data['trip_count'].rolling(window=48).mean()  # Added 48-hour MA
            ...
            data['trip_count_std_48h'] = data['trip_count'].rolling(window=48).std()  # Added 48-hour STD
            ...
            data['trip_count_diff_48'] = data['trip_count'].diff(48)  # Added 48-hour diff
    ...
    def create_sequences(self, X, y):
        """
        Modified to create sequences targeting 24 hours ahead
        """
    ...
    def fit(self, df, test_size=0.2, val_size=0.25, epochs=50, batch_size=32):
        ...
        checkpoint = ModelCheckpoint(
            'best_24hour_prediction_model.h5',
            monitor='val_loss',
            save_best_only=True,
            verbose=1
        )
    ...
    def fit_ensemble(self, processed_data, test_size=0.2, val_size=0.25, epochs=50, batch_size=32):
        ...
        self.models['weekday'] = TripPrediction24Hour(
        ...
        self.models['weekend'] = TripPrediction24Hour(
            seq_length=self.seq_length, 
            ensemble=False,
            lr=self.initial_lr
        )
    ...
    def plot_training_history(self, history):
        ...
        plt.tight_layout()
        plt.savefig('training_history_24h.png')
        plt.show()
    ...
    def plot_predictions(self, y_true, y_pred, n_samples=200):
        ...
        plt.title(f'Trip Count 24 Hours Ahead: Actual vs Predicted (First {n_samples} samples)')
        plt.xlabel('Sample Index')
        plt.ylabel('Trip Count')
        plt.legend()
        plt.savefig('prediction_results_24h.png')
        plt.show()
        ...
        plt.title('Residual Plot (24-hour prediction)')
        plt.xlabel('Predicted Values')
        plt.ylabel('Residuals')
        plt.savefig('residual_plot_24h.png')
        plt.show()
    
    def analyze_feature_importance(self, X_train, feature_names, n_top=15):
        print("Analyzing feature importance for 24-hour prediction... (this may take some time)")
        ...
        plt.title(f'Top {n_top} Feature Importances for 24-hour Prediction')
        plt.tight_layout()
        plt.savefig('feature_importance_24h.png')
        plt.show()
        
        print(f"Top 10 most important features for 24-hour prediction:")
    ...
    def predict(self, new_data):
        """
        Make predictions for 24 hours ahead using new data
        ...
        Returns:
            Predicted trip count 24 hours in the future
        """
        ...
        if self.ensemble and self.models:
            # Determine if weekday or weekend for the prediction point (24 hours ahead)
            if 'datetime' in new_data.columns:
                ...
                # Calculate the time 24 hours ahead
                ...
                # If no datetime column, use the last day_of_week value + 1 day
                if 'day_of_week' in new_data.columns:
                    last_day = new_data['day_of_week'].iloc[-1]
                    future_day = (last_day + 1) % 7  # Wrap around to 0 if it's day 6
    ...
    def load(cls, filepath):
        ...
        if os.path.exists(f"{filepath}_ensemble_config.pkl"):
            ...
            instance.prediction_horizon = config.get('prediction_horizon', 24)
        ...
        instance.prediction_horizon = saved_data.get('prediction_horizon', 24)
...
# Function to analyze feature importance for 24-hour predictions
def analyze_features_for_24h_prediction(model, df):
    """
    Perform comprehensive feature importance analysis for 24-hour predictions
    """
    ...
    feature_categories = {
        ...
        'Historical': ['trip_count_lag_1', 'trip_count_lag_2', 'trip_count_lag_3', 
                      ...
                      'trip_count_lag_24', 'trip_count_ma_24h', 'trip_count_std_24h',
                      'trip_count_lag_48', 'trip_count_ma_48h', 'trip_count_std_48h']
    }
    ...
    plt.title('Feature Importance by Category (24-hour Prediction)')
    
    # 2. Top features bar chart
    plt.subplot(1, 2, 2)
    top_n = 10
    sns.barplot(data=importance_df.head(top_n), y='Feature', x='Importance')
    plt.title(f'Top {top_n} Most Important Features (24-hour Prediction)')
    plt.tight_layout()
    plt.savefig('feature_importance_analysis_24h.png')
    plt.show()
    ...
    plt.title('Correlation Between Top Features and Trip Count (24-hour Prediction)')
    plt.tight_layout()
    plt.savefig('feature_correlation_heatmap_24h.png')
    plt.show()
    
    # Print insights
    print("\n=== 24-HOUR PREDICTION FEATURE IMPORTANCE ANALYSIS ===")
...
# Example usage:
def run_24hour_prediction_pipeline():
    # Load data
    df = pd.read_csv('hourly_trips_with_weather.csv')
    df['datetime'] = pd.to_datetime(df['datetime'], errors='coerce')
    
    print("Training 24-Hour Prediction Model")
    model_24h = TripPrediction24Hour(seq_length=24, ensemble=True)
    history = model_24h.fit(df, epochs=20, batch_size=128)
    model_24h.save('trip_prediction_24hour_model')
    
    print("\nPerforming Feature Importance Analysis")
    analyze_features_for_24h_prediction(model_24h, df)
    
    # Test prediction
    # Let's use the last 48 hours of data to predict 24 hours ahead
    test_data = df.tail(48).copy()
    prediction = model_24h.predict(test_data)
    print(f"\nPredicted trip count 24 hours ahead: {prediction:.2f}")
    
    print("\nComplete! 24-hour prediction model has been trained and saved.")


# Run this to train the model
if __name__ == "__main__":
    run_24hour_prediction_pipeline()